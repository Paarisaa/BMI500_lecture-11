...
ICD9CMCode=cal_icd9.iloc[1:-1][0]#ICD9CMCode

TotalDiag9=cal_icd9.iloc[1:-1][1]#TotalDiag of california icd9
...
for j in range(len(cal_icd9)-2):
    ...
    cal_icd9_edited[j+1]=cal_icd9[1:-1][0][j+1].split('.')[0]
...
for index, row in DFrame.iterrows():
    ...
    if cc==0:
        ...
        index=cal_icd9_edited.index(row['icd9'][1:])
        coun=cal_icd9_edited.count(row['icd9'][1:])
        for h in range(coun):
    ...
    elif cc!=1 and row['icd10']!= prev_code:
        prev_code=DFrame['icd10'][cc]
        cc+=1
        text_file.write(row['icd10']+'\t'+str(cal_icd9_edited[INDEX])+'\t'+str(maxfreq)+'\n')
        maxfreq=0
        #INDEX=0  
        if row['icd9'][1:] in cal_icd9_edited[:-2]:
            index=cal_icd9_edited.index(row['icd9'][1:])
